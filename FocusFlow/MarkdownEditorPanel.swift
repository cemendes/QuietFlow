...
struct MarkdownEditorPanel: View {
            ...
            if let task {
                MarkdownTextView(
                    ...
                    onTextChange: { newText in
                        isSaved = false
                        scheduleAutosave(taskId: task.id, content: newText)
                    }
                )
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .onAppear         { loadContent(for: task) }
                .onChange(of: task.id) { _, _ in loadContent(for: task) }
            } else {
                emptyState
            }
        }
    ...
    private func loadContent(for task: TaskItem) {
    ...
    private func scheduleAutosave(taskId: String, content: String) {
        saveDebounce?.cancel()
        saveDebounce = Task {
